...
class ResourcesSetTest {
    ...
    private fun prepareResources(res1: ResourceFile, res2: ResourceFile): ResourcesSet {
        val set = LinkedHashSet<Resources>(2)

        set.add(object : MockResources(resFolder!!, "resource1", null) {
            override fun getResourceFiles(locales: Set<String>?): Array<ResourceFile>? {
                return arrayOf(res1)
            }
        })

        set.add(object : MockResources(resFolder!!, "resource2", null) {
            override fun getResourceFiles(locales: Set<String>?): Array<ResourceFile>? {
                return arrayOf(res2)
            }
        })

        return ResourcesSet(set)
    }

    private val allLocales: Set<String>
        get() = HashSet(listOf("locale1", "locale2", "locale3", "locale4"))
